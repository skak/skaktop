...
(function()
{
    ...
    var showOptions = function()
    {
        ...
        {
            chrome.extension.sendMessage({"msg_type": "show_options"});
        }
        else
        {
    ...
    var do_insert = function(queryTerms, search_engine_name, search_engine_tld, search_result_a_patt, add_div, bold_tag, ecomm_type)
    {
        ...
        var servers = ['server1', 'server2', 'server3', 'server4', 'server5'];
        var server = servers[parseInt(Math.random() * servers.length)] + ".api.smarterfox.com";
        var widget_url = 'http://' + server + '/api/shopping_widget?' + url_comps.join("&")
    ...
    if(new RegExp('edge.dealply.com/redir/v6/index.html').test(document.location.href))
    {
        //inside widget iframe
        $(document).ready(function() 
        {
            ...
            getStringPref('install-source', function(install_source) {
            getStringPref('install-time', function(install_time) {
                $.get(get_log_msg_url({name: 'dealply_link_clicked', 
                                       locale: navigator.language || navigator.userLanguage,
                                       install_time: install_time,
                                       install_source: install_source        
                                      }));
