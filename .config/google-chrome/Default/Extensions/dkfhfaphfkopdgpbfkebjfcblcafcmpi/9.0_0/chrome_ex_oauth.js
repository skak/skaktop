 ...
 */
var gcount=0;
var newforcedtabcount=0;
var just_got_new_token=false;
...
ChromeExOAuth.initBackgroundPage = function(oauth_config) {
  ...
  console.log("in initBGpage");
  var url_match = chrome.extension.getURL(window.chromeExOAuth.callback_page);
  var tabs = {};
  gcount = 0;
  just_got_new_token=false;
  
  /*
  chrome.tabs.onUpdated.addListener(function(tabId, changeInfo, tab) {
    var background_page = chrome.extension.getBackgroundPage();
    //console.log("background_page var is: " + background_page);
 
    gcount=gcount+1;
    console.log("gcount is: " + gcount);
    console.log("window.chromeExOAuthRequestingAccess is: " + window.chromeExOAuthRequestingAccess);
    console.log("url_match is: " + url_match);
	background_page.console.log("Got Token?: " + just_got_new_token);
    ...
    if (changeInfo.url &&
        ...
        window.chromeExOAuthRequestingAccess == false) 
        	{
        	console.log("ALL IF's passed!");
        	console.log("tab's changeinfo status is: " + changeInfo.status);
      		console.log("changeinfo.url is: " + changeInfo.url);
      		console.log("changeinfo.url substr thing: " + changeInfo.url.substr(0, url_match.length));
      		console.log("url_match is: " + url_match);
      		console.log("tabs[" + tabId + "] is: " + tabs[tabId]);      		
      		
      		console.log("creating a tab...");
      	
      	if(true && (!(just_got_new_token)))
      	{
      		just_got_new_token=false;
      		if(newforcedtabcount<5) //MA added this hack -- for when infinite tab creates/removes cycle!
      			{
      		
      		if (newforcedtabcount==4) //MA says: if we got this far, likely in the infinite tab creation loop -- let Texty team know...
      			window.webkitNotifications.createNotification("error_icon.png","Contacts Sync", "MightyText likely created many tabs; however it should still let you complete the contacts sync at this point.  If you still have problems and your contacts won't sync up -- please report to MightyText team at info@textyapp.com. Error Code: CT25").show();
...
ChromeExOAuth.prototype.authorize = function(callback) {
  if (this.hasToken()) {
   	GOOGLE_CONTACTS_OAUTH_STATUS='ACTIVE_GOOGLE_CONTACTS_TOKEN_STORED_LOCALLY';
    callback(this.getToken(), this.getTokenSecret());
  } else {
    window.chromeExOAuthOnAuthorize = function(token, secret) {
      callback(token, secret);
    	};
    console.log("user does not have Contacts Token from Google in local datastore...");
    GOOGLE_CONTACTS_OAUTH_STATUS='NO_ACTIVE_TOKEN';
    console.log("should we have user acquire a new google contacts token? " + should_get_contacts_token_in_new_tab);
    gcount_auth = gcount_auth+1;
    console.log("gcount_auth is: " + gcount_auth);
    if (should_get_contacts_token_in_new_tab) // we do this as a global variable instead of a param to the function.
    		{
    		console.log("creating tab from authorize function...");
    		setTimeout("chrome.tabs.create({ 'url' : 'chrome_ex_oauth_new.html' });",500);
 			}         
          }
};
...
ChromeExOAuth.initCallbackPage = function() {
  ...
  background_page.console.log("in INIT-CALLBACK-PAGE");
  oauth.initOAuthFlow(function (token, secret) {
    ...
    chrome.tabs.getSelected(null, function (tab) {
      ...
      background_page.chrome.tabs.create({ 'url' : 'popup.html' }); ///token success? ma added
...
ChromeExOAuth.prototype.setToken = function(token) {
  ...
  var background_page = chrome.extension.getBackgroundPage();
  background_page.console.log("STORED CONTACTS TOKEN!");
  background_page.chrome.tabs.create({ 'url' : 'http://www.google.com' },
  
  function(tab) {
			background_page.console.log("New tabs url right when set_token is: " + tab.url);
			background_page.just_got_new_token=true;
});
...
ChromeExOAuth.prototype.getAccessToken = function(oauth_token, oauth_verifier,
  ...
  if (bg.chromeExOAuthRequestingAccess == false) {
    ...
    ChromeExOAuth.sendRequest("GET", result.signed_url, null, null, onToken);
  	console.log("calling get on Oauth token");
  }
};
