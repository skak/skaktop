...
function Auth() {
  ...
  // @TODO: This isn't actually used currently.
  ...
  function login(doWebClient, _username, _password, _remember, callback) {
    ...
    getRpc().client.UserStore.authenticate(handleLogin, username, password,
      "en-jsclipper", "98167d78f2beb2e0");
    if (doWebClient) {
      loginWebClient(_username, _password, _remember);
    }
  ...
  function failLogin(exception) {
    ...
    function postLogout() {
      for (var i = 0; i < callbacks.length; i++) {
        runCallback(i, {authenticationToken: null, displayName: originalUser, error: error});
      }
      callbacks = [];
  ...
  function handleLogin(result, exception) {
    ...
    if (result) {
      ...
      userInfo[lookupName].authTimeOffset = (new Date().getTime()) - result.currentTime;
  ...
  function authFresh() {
    if (!getCurrentUserAuthResult().expiration) return false;
    ...
    // made).
    if (now > (getCurrentUserAuthResult().expiration - (3 * 60 * 1000))) return false;
    return true;
  }

  function bizAuthFresh() {
    if (!getCurrentUserBizAuthResult().expiration) return false;
    var now = new Date().getTime();
    now -= authTimeOffset;

    // We'll say that our auth is stale if we're within three minutes of having
    // it expire. This avoids the problem of finding that it's fresh but
    // expiring in a second or two and having it expire by the time a clip is
    // actually made).
    if (now > (getCurrentUserBizAuthResult().expiration - (3 * 60 * 1000))) {
      return false;
    }
    return true;
  }
