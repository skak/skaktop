function Extension() {
  ...
  function showIntro() {
    ...
    if (!Persistent.get(introShown)) {
      var introPageUrl = bootstrapInfo.get("marketingUrl") + "/webclipper/";
      if (!/china/i.test(bootstrapInfo.getName())) {
        introPageUrl += "guide/"
      }
  ...
  function clipPdfFromTab(tab, note) {
    ...
    Browser.sendToTab(tab, { name: "clipPdf", note: note, notify: options.get("clipNotificationEnabled") });
  }
  ...
  Browser.addMessageHandlers({
    ...
    isClearlySimSearchRunning: msgHandlerCheckClearlySimSearch,
  ...
  function msgHandlerLogin(request, sender, sendResponse) {
    ...
    if (request.fromUrl) {
      auth.login(false, request.data.username, request.data.password,
        request.data.remember, sendResponse);
    }
    else {
      auth.login(true, request.data.username, request.data.password,
        request.data.remember, sendResponse);
    }
  }
    ...
    */
    ...
    function receivePersonalSmartFilingInfo(response, exception) {
      if (response) {
        ...
        if ((authInfo.bizAuthenticationToken && bizRelatedNotes && linkedNotebooks && !updatingLinkedNotebooks
            && haveUpToDateLinkedNotebooks()) || !authInfo.bizAuthenticationToken) {
          sendRelatedNotes();
        }
      }
      else {
    ...
    function getFilingRecommendationRequest(type) {
      var resultSpec = {
        includeTitle: true,
        includeUpdated: true,
    ...
    function getRelatedPersonalNotes() {
      JsonQueue.handleExpensiveOpRequest(rpc.shardId, rpc.client.NoteStoreExtra.getFilingRecommendations,
        receivePersonalSmartFilingInfo, authInfo.authenticationToken, getFilingRecommendationRequest("personal"));
    }
    ...
    function getRelatedBizNotes() {
      JsonQueue.handleExpensiveOpRequest(bizRpc.shardId, bizRpc.client.NoteStoreExtra.getFilingRecommendations,
        receiveBizSmartFilingInfo, authInfo.bizAuthenticationToken, getFilingRecommendationRequest("biz"));
      if (!linkedNotebooks) {
        updatingLinkedNotebooks = true;
        linkedNotebooks = new LinkedNotebooks(authInfo.authenticationToken, rpc, receiveLinkedNotebooks);
      }
  ...
  function msgHandlerCheckClearlySimSearch(request, sender, sendResponse) {
    Browser.sendToTab(sender.tab, {name: "clearlySimSearchEnabled", enabled: false});
  }
