...
function Popup() {
  ...
  function keyboardUserSwitch(evt) {
    if (document.querySelector("#switchFreeView").className == "view") { // disable for the upsell dialog
  ...
  */
  ...
  function showGlobalError(messageKey, substitutions) {
    hideView("waiting");
    var message;
  ...
  function getSmartFilingInfo() {
    ...
    if (!setting != "NONE") {
      ...
      if (pageInfo.recommendationText) {
        ...
        JsonQueue.handleExpensiveOpRequest(jsonRpc.shardId, jsonRpc.client.NoteStoreExtra.getFilingRecommendations,
          receivePersSmartFilingInfo, authenticationToken, filingRecommendationRequest);
        if (bizAuthenticationToken) {
          ...
          JsonQueue.handleExpensiveOpRequest(bizRpc.shardId, bizRpc.client.NoteStoreExtra.getFilingRecommendations,
            receiveBizSmartFilingInfo, bizAuthenticationToken, filingRecommendationRequest);
        }
      }
      else {
  ...
  function receiveSmartFilingInfo(data) {
    if (data) {
      ...
      log.warn("error getting smart filing info.");
    }
    finishNotebookLoading();
  }

  function receivePersSmartFilingInfo(data) {
  ...
  function checkPageReady() {
    ...
    Browser.sendToTab(tab, {name: "content_ready_5_9_5"});
