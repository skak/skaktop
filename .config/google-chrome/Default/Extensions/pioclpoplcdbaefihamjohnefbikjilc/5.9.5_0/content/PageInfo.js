function PageInfo() {
  ...
  if (typeof Browser != "undefined") {
    Browser.addMessageHandlers({
      ...
      content_ready_5_9_5: readyRequestHandler
    });
  }
  else {
  ...
  function matchesAllowedSelectorInSearchEngine(node) {
    var selector = searchEngines[searchEngine].allowedElements.join(", ");
  ...
  function matchesBannedSelectorInSearchEngine(node) {
    if (searchEngines[searchEngine].bannedSubelements) {
      var selector = searchEngines[searchEngine].bannedSubelements.join(", ");
      if (node.webkitMatchesSelector && node.webkitMatchesSelector(selector)) {
        return true;
      }
    }
