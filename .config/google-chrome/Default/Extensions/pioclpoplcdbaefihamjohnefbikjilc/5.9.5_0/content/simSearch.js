...
var searchEngines = {
  Baidu: {
    ...
    "insertBefore": [
      ...
      "#container" // error or no results found.
    ]
  },
  Bing: {
  ...
  Naver: {
    ...
    "queryParam": "query",
    "insertBefore": [
      ".section:not(.ad_power) > .section_head"
    ]
  },
  Yahoo: {
  ...
  YandexRU: {
    ...
    "queryParam": "text",
    "insertBefore": [".b-serp-list", ".b-serp-item__wrapper"]
  }
};
...
function checkSimSearch() {
  ...
  for (var engine in searchEngines) {
    if (searchEngines[engine].regexp.test(href)){
      // Hack to disable sim search in gmail because it causes conflicts with some other extensions there, notably this
      // one: http://www.activeinboxhq.com/
      if (href.match(/^https?:\/\/mail\.google\./i)) {
        continue;
      }
...
function SearchHelper() {
  ...
  function checkClearlySimSearch() {
    if (SAFARI) {
      startSearch();
    }
    else {
      Browser.sendToExtension({name: "isClearlySimSearchRunning"});
    }
  }

  function msgHandlerClearlySimSearchEnabled(response) {
    if (!response.enabled) {
      startSearch();
    }
  }
  ...
  Browser.addMessageHandlers({
    ...
    clearlySimSearchEnabled: msgHandlerClearlySimSearchEnabled
  });
  ...
  if (searchEngine == "Google") {
    ...
    // loaded
    document.addEventListener("DOMNodeInserted", function(evt) { if(evt.srcElement.id == "ires") checkClearlySimSearch() });
    ...
    document.querySelector(".b-form-button__input").addEventListener("click", checkClearlySimSearch);
  }
  else {
