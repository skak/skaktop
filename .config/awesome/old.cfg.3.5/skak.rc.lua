...
beautiful.init(awful.util.getdir("config") .. "/themes/zenburn/theme.lua") -- THEME DECLARATION
local naughty = require("naughty")
local menubar = require("menubar")
local wi = require("wi")
if awesome.startup_errors then -- Check if awesome encountered an error during startup and fell back to another config.
...
terminal = "xfce4-terminal"					-- VARIABLES
editor = "gedit" or os.getenv("EDITOR")
...
mytimer:start()
--]]
local layouts = { awful.layout.suit.tile,            -- 2 TILE
...
myawesomemenu = {						-- MENU
   ...
   { "SkAk config", editor .. " " .. "/home/skak/.config/awesome/skak.rc.lua" }, 
   { "restart", awesome.restart },
   { "quit", awesome.quit },
   { "FALLBACK config", editor .. " " .. "/home/skak/.config/awesome/rc.fallback.lua" }
}
myusermenu = {{ "$::terminal", terminal },			-- USER MENU
              { "$::google chrome", browser },
              { "$::gedit", gedit },
              { "$::pidgin", pidgin },
              { "$::fileMan", thunar },
              { "$::azureus vuze", vuze }}
myrootmenu = {{ "#::terminal", "gksudo terminal" },		-- ROOT MENU
              { "#::gedit", "gksudo gedit" },
              { "#::fileMan", "gksudo thunar" }}
mymainmenu = awful.menu({ items = {{ "awesome", myawesomemenu, beautiful.sky_awesome_icon },
				   { "user", myusermenu, beautiful.sky_awesome_icon },
				   { "root", myrootmenu, beautiful.sky_awesome_icon }} })
mylauncher = awful.widget.launcher({ image = beautiful.awesome_icon, menu = mymainmenu })
skaklauncher = awful.widget.launcher({ image = beautiful.sky_awesome_icon, menu = mymainmenu })
