...
window.addEventListener("DOMContentLoaded", function() {
  ...
  if (/china/i.test(locale)) {
    document.querySelector(".branding img").src = "../images/simsearch_logo_china.png";
  }
  ...
  if (request.relatedNotes) {
    ...
    if (request.type === "account") {
      ...
      if (request.isBizUser) {
        sectionTitle = Browser.i18n.getMessage("searchHelperTopMatchesBiz", [notes.length]);
      }
      else {
        sectionTitle = Browser.i18n.getMessage("searchHelperTopMatchesPers", [notes.length]);
      }
